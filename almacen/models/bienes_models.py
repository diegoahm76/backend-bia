...
class MetodosValoracionArticulos(models.Model):
    cod_metodo_valoracion = models.SmallIntegerField(primary_key=True, db_column='T058CodMetodoValoracion')
    nombre = models.CharField(max_length=50, db_column='T058nombre', unique=True)
    descripcion = models.CharField(max_length=255, db_column='T058descripcion', blank=True, null=True)
...
class TiposDepreciacionActivos(models.Model):
    cod_tipo_depreciacion = models.SmallIntegerField(primary_key=True, db_column='T059CodTipoDepreciacion')
...
class CatalogoBienes(models.Model):
    ...
    cod_tipo_activo = models.ForeignKey(TiposActivo, on_delete=models.SET_NULL, null=True, blank=True, db_column='T057Cod_TipoActivo')
    nivel_jerarquico = models.SmallIntegerField(validators=[MaxValueValidator(5), MinValueValidator(1)], db_column='T057nivelJerarquico')
    ...
    cod_metodo_valoracion = models.ForeignKey(MetodosValoracionArticulos, on_delete=models.SET_NULL, null=True, blank=True, db_column='T057Cod_MetodoValoracion')
    cod_tipo_depreciacion = models.ForeignKey(TiposDepreciacionActivos, on_delete=models.SET_NULL, null=True, blank=True, db_column='T057Cod_TipoDepreciacion')
    cantidad_vida_util = models.SmallIntegerField(null=True, blank=True, db_column='T057cantidadVidaUtil')
    ...
    valor_residual = models.DecimalField(null=True, blank=True, max_digits=10, decimal_places=0, db_column='T057valorResidual')
    stock_minimo = models.SmallIntegerField(null=True, blank=True, db_column='T057stockMinimo')
    stock_maximo = models.IntegerField(null=True, blank=True, db_column='T057stockMaximo')
    solicitable_vivero = models.BooleanField(default=False, db_column='T057solicitablePorVivero')
    es_semilla_vivero = models.BooleanField(null=True, blank=True, db_column='T057esSemillaMVVivero')
...
class EstadosArticulo(models.Model):
    cod_estado = models.CharField(max_length=1, primary_key=True, db_column='T051Cod_Estado')
    nombre = models.CharField(max_length=20, unique=True, db_column='T051nombre')
...
class EntradasAlmacen(models.Model):
    ...
    numero_entrada_almacen = models.IntegerField(unique=True, db_column='T063nroEntradaAlmacen')	
    ...
    id_persona_anula=models.ForeignKey('transversal.Personas',related_name='persona_anula_entrada',on_delete=models.SET_NULL, blank=True, null=True, db_column='T063Id_PersonaAnula')
...
class ItemEntradaAlmacen(models.Model):
    ...
    valor_unitario = models.DecimalField(max_digits=11, decimal_places=2, db_column='T064valorUnitario')
    porcentaje_iva = models.ForeignKey(PorcentajesIVA, on_delete=models.CASCADE, db_column='T064Id_PorcentajeIVA')
    valor_iva = models.DecimalField(max_digits=11, decimal_places=2, db_column='T064valorIVA')
    valor_total_item = models.DecimalField(max_digits=11, decimal_places=2, db_column='T064valorTotalItem')
    id_bodega = models.ForeignKey(Bodegas, on_delete=models.CASCADE, db_column='T064Id_Bodega')
    cod_estado = models.ForeignKey(EstadosArticulo, on_delete=models.SET_NULL, blank=True, null=True, db_column='T064Cod_Estado')

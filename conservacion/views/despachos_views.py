...
class CreateDespacho(generics.UpdateAPIView):
    ...
    def put(self,request):
        ...
        if items_despacho == None:
            return Response({'success':False,'detail':'El despacho debe tener por lo menos un item' },status=status.HTTP_404_NOT_FOUND)
        ...
        if len(set(validacion_ceros)) <= 1 and len(items_despacho)!=1:
            return Response({'success':False,'detail':'No todos las cantidades despachadas de los items pueden estar en cero'},status=status.HTTP_404_NOT_FOUND)
        
        # SE VALIDA QUE EL DESPACHO TENGA AL MENOS UN ITEMS
        if len(items_despacho) == 0:
            return Response({'success':False,'detail':'Se debe despachar al menos un item'},status=status.HTTP_404_NOT_FOUND)
        ...
        for i in items_despacho:
            ...
            if not instancia_bien:
                return Response({'success':False,'detail':'El bien con número de posición (' + str(i['nro_posicion_en_despacho']) + ') no existe' },status=status.HTTP_404_NOT_FOUND)
            ...
            if not instancia_inventario:
                return Response({'success':False,'detail':'El bien (' + instancia_bien.nombre + ') con número de posición (' + str(i['nro_posicion_en_despacho']) + ') no existe en el inventario del vivero.'},status=status.HTTP_404_NOT_FOUND)
            
            if ([i['id_bien'], i["agno_lote"], i["nro_lote"], i["cod_etapa_lote"]] in aux_validacion_bienes_despachados_repetidos):
                return Response({'success':False,'detail':'Error en el bien (' + instancia_bien.nombre + ') con número de posición (' + str(i['nro_posicion_en_despacho']) + '). No se puede ingresar dos veces el mismo bien.'},status=status.HTTP_404_NOT_FOUND)
            ...
            if cantidad_disponible <= 0:
                return Response({'success':False,'detail':'El bien (' + instancia_bien.nombre + ') con número de posición (' + str(i['nro_posicion_en_despacho']) + ') no tiene cantidad disponible.' },status=status.HTTP_404_NOT_FOUND)
            ...
            if aux_cantidad_total[str(i['id_bien'])] > int(i['cantidad_solicitada']):
                return Response({'success':False,'detail':'Error en el bien (' + instancia_bien.nombre + ') con número de posición (' + str(i['nro_posicion_en_despacho']) + '). La suma de las cantidades despachadas de este bien supera la cantidad solicitada.' },status=status.HTTP_404_NOT_FOUND)
            ...
            if instancia_bien.nivel_jerarquico > 5 or instancia_bien.nivel_jerarquico < 2:
                return Response({'success':False,'detail':'Error en el bien (' + instancia_bien.nombre + ') con número de posición (' + str(i['nro_posicion_en_despacho']) + '). El bien no es de nivel 2 al 5' },status=status.HTTP_404_NOT_FOUND)
            
            # SE VALIDA QUE EL BIEN SEA DE CONSUMO
            if instancia_bien.cod_tipo_bien != 'C':
                return Response({'success':False,'detail':'El bien (' + instancia_bien.nombre + ') no es de consumo' },status=status.HTTP_404_NOT_FOUND)
            ...
            if (instancia_bien.es_semilla_vivero == True and instancia_bien.cod_tipo_elemento_vivero == "MV") or instancia_bien.cod_tipo_elemento_vivero == "HE":
                return Response({'success':False,'detail':'Error en el bien (' + instancia_bien.nombre + ') con número de posición (' + str(i['nro_posicion_en_despacho']) + '). Solo se pueden despachar plantas que se encuentren en distribución o insumos.' },status=status.HTTP_404_NOT_FOUND)
            
            if instancia_bien.cod_tipo_elemento_vivero == 'IN' and (i['agno_lote'] != None or i['nro_lote'] != None or i['cod_etapa_lote'] != None):
                return Response({'success':False,'detail':'Error en el bien (' + instancia_bien.nombre + ') con número de posición (' + str(i['nro_posicion_en_despacho']) + '). Si el bien a despachar es tipo insumo debe dejar los campos (agno_lote), (nro_lote) y (cod_etapa_lote) en null.' },status=status.HTTP_404_NOT_FOUND)
            ...
            if instancia_bien.cod_tipo_elemento_vivero == 'MV' and instancia_bien.es_semilla_vivero == False and (i['agno_lote'] == None or i['nro_lote'] == None or i['cod_etapa_lote'] == None or i['agno_lote'] == '' or i['nro_lote'] == '' or i['cod_etapa_lote'] == ''):
                return Response({'success':False,'detail':'Error en el bien (' + instancia_bien.nombre + ') con número de posición (' + str(i['nro_posicion_en_despacho']) + '). Si el bien a despachar es tipo planta debe ingrasar los campos (agno_lote), (nro_lote) y (cod_etapa_lote).' },status=status.HTTP_404_NOT_FOUND)
            ...
            if i['cantidad_despachada'] == 0:
                return Response({'success':False,'detail':'Error en el bien (' + instancia_bien.nombre + ') con número de posición (' + str(i['nro_posicion_en_despacho']) + '). Si la cantidad despachada es igual a cero no debe ingresar ninguna subfila' },status=status.HTTP_404_NOT_FOUND)
            
            # SE VALIDA QUE CUANDO SE INGRESE UNA CANTIDAD DESPACHADA IGUAL A CERO SE CUMPLAN LOS REQUERIMIENTOS EXIGIDOS POR MODELADO
            if i['cantidad_despachada'] >= i['cantidad_solicitada']:
                return Response({'success':False,'detail':'Error en el bien (' + instancia_bien.nombre + ') con número de posición (' + str(i['nro_posicion_en_despacho']) + '). La cantidad despachada debe ser menor a la cantidad solicitada.' },status=status.HTTP_404_NOT_FOUND)
...
class UpdatePreparacionMezclas(generics.UpdateAPIView):
    ...
    def put(self, request):
        ...
        if not instancia_despacho:
            return Response({'success':False, 'detail':'No se encontró el despacho que desea actualizar.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if fecha_despacho < fecha_posible:
            return Response({'success':False,'detail':'No es posible actualiazar un despacho con más de 9 días de anterioridad.'}, status=status.HTTP_400_BAD_REQUEST)
...
#----------------------------------------------------> VALIDACIONES DE LOS ITEMS A ACTUALIZAR USADOS EN LA PREPARACION <----------------------------#
        for i in items_actualizar:
            ...
            if not instancia_items_despacho:
                return Response({'success':False,'detail':'El bien con número de posición ' + str(i['nro_posicion_en_despacho']) + ' no existe el registro del despacho.'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if not instancia_bien:
                return Response({'success':False,'detail':'El bien con número de posición ' + str(i['nro_posicion_en_despacho']) + ' no existe registrado en el sistema como bien.'}, status=status.HTTP_400_BAD_REQUEST)
...
class GetSolicitudesVivero(generics.ListAPIView):
    ...
    def get(self,request):
        ...
        if not fecha_despacho or fecha_despacho == '':
            return Response({'success':False,'detail':'Es obligatorio ingresar una fecha de despacho.' },status=status.HTTP_404_NOT_FOUND)
        
        fecha_despacho = datetime.strptime(request.query_params.get('fecha_aprobado'), "%Y-%m-%d %H:%M:%S")
        
        if not id_vivero or id_vivero == '':
            return Response({'success':False,'detail':'Es obligatorio seleccionar un vivero.' },status=status.HTTP_404_NOT_FOUND)
        ...
        if nro_solicitud:
            ...
            if not instancia_solicitudes:
                return Response({'success':False,'detail':'No se encontraron coincidencias con ese número de solicitud' },status=status.HTTP_404_NOT_FOUND)
            
            instancia_solicitudes = instancia_solicitudes.filter(id_vivero_solicitud=id_vivero)
            if not instancia_solicitudes:
                return Response({'success':False,'detail':'El número de solicitud ingresado no está registrada dentro del vivero ingresado'},status=status.HTTP_404_NOT_FOUND)
            ...
            if not instancia_solicitudes:
                return Response({'success':False,'detail':'La fecha del despacho debe ser superior a la fecha de la solicitud'},status=status.HTTP_404_NOT_FOUND)
        ...
        # SI SE NO INGRESA NÚMERO DE SOLICITUD SE BUSCA POR EL ID VIVERO Y QUE LA SOLICITUD SEA MENOR LA FECHA DE DESPACHO
            ...
            if not instancia_solicitudes:
                return Response({'success':False,'detail':'El viero ingresado no tiene solicitudes registradas o no existe.' },status=status.HTTP_404_NOT_FOUND)
...
class GetItemsSolicitud(generics.ListAPIView):
    ...
    def get(self, request,id_solicitud_viveros):
        ...
        if not items_solicitud:
            return Response({'success':False,'detail':'La solicitud seleccionada no tiene items registrados.' },status=status.HTTP_404_NOT_FOUND)
...
class GetInsumo(generics.ListAPIView):
    ...
    def get(self,request,id_vivero,codigo_insumo):
        ...
        if not instancia_bien:
            return Response({'success':False,'detail':'El bien ingresado no es de tipo insumo o no está registrado en Catalogo de bienes.' },status=status.HTTP_404_NOT_FOUND)
        ...
        if not bien_inventario:
                return Response({'success':False,'detail':'El bien ingresado no existe en el inventario del vivero.' },status=status.HTTP_404_NOT_FOUND)
            
        cantidad_disponible = UtilConservacion.get_cantidad_disponible_F(instancia_bien, bien_inventario) 
        
        if cantidad_disponible <= 0:
            return Response({'success':False,'detail':'Este insumo no tiene cantidad disponible.' },status=status.HTTP_404_NOT_FOUND)
...
class GetPlanta(generics.ListAPIView):
    ...
    def get(self,request):
        ...
        if not id_vivero or id_vivero == '':
            return Response({'success':False,'detail':'Es obligatorio ingresar un vivero.' },status=status.HTTP_404_NOT_FOUND)
        
        if not codigo_bien or codigo_bien == '':
            return Response({'success':False,'detail':'Es obligatorio ingresar el código del bien.' },status=status.HTTP_404_NOT_FOUND)
        
        if not nombre or nombre == '':
            return Response({'success':False,'detail':'Es obligatorio ingresar el nombre del bien un vivero.' },status=status.HTTP_404_NOT_FOUND)
        ...
        if not instancia_bien:
            return Response({'success':False,'detail':'El bien ingresado no es una planta o no está registrado en Catalogo de bienes.' },status=status.HTTP_404_NOT_FOUND)
        ...
        if not bien_inventario:
            return Response({'success':False,'detail':'La planta ingresada no existe en el inventario del vivero o no hay ningún lote en etapa de distribución.' },status=status.HTTP_404_NOT_FOUND)
...
class GetDespachosVivero(generics.ListAPIView):
    ...
    def get(self, request):
        ...
        if fecha_desde == '' or fecha_desde == None:
            return Response({'success':False,'detail':'La fecha desde es obligatoria.' },status=status.HTTP_404_NOT_FOUND)
        if fecha_hasta == '' or fecha_hasta == None:
            return Response({'success':False,'detail':'La fecha hasta es obligatoria.' },status=status.HTTP_404_NOT_FOUND)
        if fecha_desde != '':
...
class GetItemsDespacho(generics.ListAPIView):
    ...
    def get(self, request,id_despacho_vivero):
        ...
        if not items_despacho:
            return Response({'success':False,'detail':'El despacho seleccionad0 no tiene items registrados.' },status=status.HTTP_404_NOT_FOUND)
...
class AnularPreparacionMezclas(generics.UpdateAPIView):
    ...
    def put(self, request, id_despacho_anular):
        ...
        if not despacho_a_anular:
            return Response({'success':False,'detail':'No se encontró ningun despacho con el id que ingresó'}, status=status.HTTP_400_BAD_REQUEST)
        
        if despacho_a_anular.despacho_anulado == True:
            return Response({'success':False,'detail':'Este despacho ya fue anulado.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not items_despacho_a_anular:
            return Response({'success':False,'detail':'Este despacho no registra items.'}, status=status.HTTP_400_BAD_REQUEST)
        
        # SE VALIDA LA FECHA
        fecha_posible = datetime.now() - timedelta(days=9)
        if despacho_a_anular.fecha_despacho < fecha_posible:
            return Response({'success':False,'detail':'No es posible anular un despacho que tenga más de 9 días de anterioridad.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not instancia_solicitud:
            return Response({'success':False,'detail':'No se encontró ninguna solicitud asociada con el despacho ingresado.'}, status=status.HTTP_400_BAD_REQUEST)
